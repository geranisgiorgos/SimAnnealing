function [minimum,fval] = anneal(loss, parent, A, C, eqin, foit, kath, options)
...
def = struct(...
        'CoolSched',@(T) (.8*T),...
        'Generator',@(x) (x+(randperm(length(x))==length(x))*1),...
        'Generator2',@(x) (x+(randperm(length(x))==length(x))*(-1)),...
        'InitTemp',1,...
        'MaxConsRej',1000,...
        'MaxSuccess',20,...
        'MaxTries',300,...
...
% πτυχιακές επιτρεπόμενες από καθηγητές
ptyx = C(foit+1:length(C));
